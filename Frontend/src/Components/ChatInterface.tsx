...
const ChatInterface = () => {
  ...
  // ✅ Single bubble for continuous SSE updates
  const upsertCurrentStepMessage = (stepData: any) => {
    ...
    if (title) {
      ...
      displayText = formatActivity(stepData);
    }

    if (!displayText) return;

    setMessages((prev) => {
      const lastStepIdx = [...prev]
        .map((m, i) => ({ i, m }))
        .filter(({ m }) => m.sender === "assistant" && m._currentStep === true)
        .map(({ i }) => i)
        .pop();
      ...
      if (lastStepIdx == null) {
        return [
          ...prev,
          { sender: "assistant", text: displayText, _currentStep: true },
        ];
      }
      const next = [...prev];
      next[lastStepIdx] = { ...next[lastStepIdx], text: displayText };
      return next;
    });
  };

  const removeCurrentStepMessage = () => {
    setMessages((prev) => prev.filter((m) => !m._currentStep));
  };
    ...
    try {
      ...
      es.addEventListener("step", (ev: MessageEvent) => {
        try {
          ...
          upsertCurrentStepMessage(data);
          const t = data.delta || data.text;
          if (t) upsertStreamingAssistant(t);
        } catch {
          upsertCurrentStepMessage({ status: "Working…" });
        }
      });
      ...
      es.addEventListener("response", (ev: MessageEvent) => {
        try {
          ...
          finalizeStreamingAssistant(
            data?.response || data?.text || data?.result || ""
          );
        } catch {
          removeCurrentStepMessage();
          finalizeStreamingAssistant("");
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-300 max-w-3xl mx-auto">
            Get instant insights about CSE stocks, market trends, and company
                    ...
                    <div
                      className={`px-5 py-3 rounded-2xl shadow-md ${
                        msg.sender === "user"
                          ...
                          : msg._currentStep
                          ? "bg-blue-50 text-blue-800 rounded-bl-none border border-blue-200 text-sm italic"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
          <div className="bg-slate-800 rounded-xl p-6 border border-yellow-500 border-opacity-30">
            ...
            <p className="text-gray-400 text-sm">
              Get instant insights on stock movements and market trends.
            </p>
          </div>
          ...
          <div className="bg-slate-800 rounded-xl p-6 border border-yellow-500 border-opacity-30">
            ...
            <p className="text-gray-400 text-sm">
              Detailed information about listed companies and their performance.
            </p>
          </div>
          ...
          <div className="bg-slate-800 rounded-xl p-6 border border-yellow-500 border-opacity-30">
            ...
            <p className="text-gray-400 text-sm">
              AI-powered suggestions based on market conditions and trends.
            </p>
          </div>
        </div>
      </div>
